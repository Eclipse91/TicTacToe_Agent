...
def get_cudnn_version():
    ...
    '''
    try:
        location = find_cudnn_version_file().split('\n')[0]
        cudnn_version = ''
        ...
        if len(cudnn_version) > 2:
            cudnn_version = '.'.join(extract_numbers(cudnn_version))
...
def log_gpu_info():
    ...
    if physical_devices:
        ...
        driver_info = f'\n{"="*10}NVIDIA Driver Informations{"="*10}     {get_nvidia_driver_info()}\n' if get_nvidia_driver_info() else ''
        cuda_info = f'{"="*10}CUDA Toolkit Runtime API Version{"="*10}\n{get_cuda_info()}\n' if get_cuda_info() else ''
        cudnn_info = f'{"="*10}cuDNN Version{"="*10}\n{get_cudnn_version()}\n'if get_cudnn_version() else ''
        ...
        logging.info(box_text(box_content))
    else:
